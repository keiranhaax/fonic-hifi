...
public final class MetadataExtractionService: ObservableObject, Sendable {
    ...
    public func extractMetadata(from urls: [URL]) async throws -> [TrackMetadata] {
        return try await withThrowingTaskGroup(of: TrackMetadata?.self) { group in
...
public enum MetadataExtractionError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .extractionFailed(let url, let error):
            return "Failed to extract metadata from \(url.lastPathComponent): \(error.localizedDescription)"
        }
    }
}