...
public final class AudioMonitor: ObservableObject, AudioMonitoringService {
    ...
    public func getHistoricalMetrics(from startTime: Date, to endTime: Date) async -> [AudioMetrics] {
        metricsHistory.filter { metrics in
    ...
    public func performDiagnosticsCheck() async -> PlaybackDiagnostics {
        ...
        return await PlaybackDiagnostics(
            ...
            qualityAssessment: assessAudioQuality(),
            efficiencyAnalysis: analyzeEfficiency(),
            activeIssues: identifyActiveIssues(),
            recentAlerts: Array(alertHistory.suffix(10)),
            recommendations: generateRecommendations(),
            optimizations: identifyOptimizations(),
            sessionStatistics: createSessionStatistics(),
            errorHistory: createErrorHistory(),
            milestones: identifyMilestones(),
            osCompatibility: assessOSCompatibility(),
            hardwareCompatibility: assessHardwareCompatibility(),
            formatSupport: analyzeFormatSupport(),
            debugInfo: collectDebugInfo(),
            logEntries: collectRecentLogEntries(),
            configurationDump: createConfigurationDump(),
        )
    }
    ...
    public func getSystemAudioMetrics() async -> SystemAudioMetrics {
        await systemMetricsCollector.collectSystemMetrics()
    }
    ...
    public func getProfilingResults() async -> PerformanceProfile? {
        guard let profilingData,
              let startTime = profilingStartTime
        else {
            return nil
        }
        ...
        return await PerformanceProfile(
            startTime: startTime,
            duration: duration,
            ...
            bottlenecks: identifyBottlenecks(from: profilingData),
            optimizations: identifyOptimizationsFromProfiling(profilingData),
        )
    }
...
private extension AudioMonitor {
    ...
    func handleAudioInterruption(_ notification: Notification) async {
        ...
        let interruptionType: InterruptionType = switch type {
        case .began:
            .began
        case .ended:
            .ended
    ...
    func calculateAverageBufferFill() -> Float {
        let fills = metricsHistory.map(\.bufferFillLevel)
    ...
    func calculateAverageMetrics() -> AudioMetrics {
        ...
        let avgCPU = metricsHistory.map(\.cpuUsage).reduce(0, +) / count
        let avgMemory = metricsHistory.map(\.memoryUsage).reduce(0, +) / Int64(metricsHistory.count)
        let avgLatency = metricsHistory.map(\.renderLatency).reduce(0, +) / Double(metricsHistory.count)
        let avgBufferFill = metricsHistory.map(\.bufferFillLevel).reduce(0, +) / count
    ...
    func calculatePeakMetrics() -> AudioMetrics {
        ...
        let maxCPU = metricsHistory.map(\.cpuUsage).max() ?? 0
        let maxMemory = metricsHistory.map(\.memoryUsage).max() ?? 0
        let maxLatency = metricsHistory.map(\.renderLatency).max() ?? 0
        let minBufferFill = metricsHistory.map(\.bufferFillLevel).min() ?? 1.0
        let maxUnderruns = metricsHistory.map(\.bufferUnderruns).max() ?? 0
        let maxDroppedFrames = metricsHistory.map(\.droppedFrames).max() ?? 0
    ...
    func calculateSessionPerformanceScore() -> Double {
        ...
        let avgPerformanceScore = metricsHistory.map(\.performanceScore).reduce(0, +) / Float(metricsHistory.count)
...
private class InterruptionStatsTracker {
    ...
    func getStatistics() async -> InterruptionStatistics {
        ...
        let avgDuration = interruptions.map(\.duration).reduce(0, +) / Double(max(1, interruptions.count))
        let maxDuration = interruptions.map(\.duration).max() ?? 0
        let successRate = Double(interruptions.count(where: { $0.recoverySuccessful })) / Double(max(1, interruptions.count))
...
private extension AudioMonitor {
    func setupEngineSpecificMonitoring(_: AudioEngineService) async {
            ...
            if latest.jitter > 0.004 {
                recommendations.append(
                    PerformanceRecommendation(
                        ...
                        description: "Detected jitter of \(String(format: "%.3f", latest.jitter * 1000)) ms. Wired output stabilizes the master clock.",
    ...
    func identifyOptimizations() async -> [OptimizationOpportunity] {
        ...
        if let profilingData {
            await opportunities.append(contentsOf: identifyOptimizationsFromProfiling(profilingData))
        }
        if let latest = metricsHistory.last {
    ...
    func createSessionStatistics() async -> SessionStatisticsSummary {
        ...
        let qualityDrops = metricsHistory.count(where: { $0.qualityScore < 0.85 })
    ...
    func collectDebugInfo() async -> DebugInformation {
        ...
        let bufferAverage = metricsHistory.isEmpty ? 1.0 : Double(metricsHistory.map(\.bufferFillLevel).reduce(0, +)) / Double(metricsHistory.count)
    ...
    func generateKeyFindings(metrics: [AudioMetrics], alerts: [PlaybackAlert]) async -> [String] {
        ...
        let avgCPU = metrics.map(\.cpuUsage).reduce(0, +) / Float(max(1, metrics.count))
        if avgCPU > 50 {
    ...
    private func makePerformanceTrend(label: String, indicator: TrendIndicator, higherIsBetter _: Bool) -> PerformanceTrend {
