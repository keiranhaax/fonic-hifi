...
public struct BitPerfectValidationResult: Sendable, Equatable {
    ...
    public var criticalParametersMatch: Bool {
        sampleRateMatches && bitDepthMatches && channelCountMatches
    }

    /// Whether volume settings allow bit-perfect playback
    public var volumeIsOptimal: Bool {
        systemVolume == 1.0 && applicationVolume == 1.0
    }
...
public enum BitPerfectMismatchReason: String, Sendable, CaseIterable {
    ...
    public var userFriendlyDescription: String {
        ...
        case .bitDepthMismatch:
            "Bit depth conversion is required"
        case .channelCountMismatch:
            "Channel configuration doesn't match"
        case .deviceNotCapable:
            "Output device doesn't support the format"
        case .systemVolumeNotUnity:
            "System volume is not at 100%"
        case .applicationVolumeNotUnity:
            "Application volume is not at 100%"
        case .audioProcessingActive:
            "Audio processing is affecting the signal"
        case .formatConversionRequired:
            "Audio format conversion is required"
        case .systemMixerActive:
            "System audio mixer is processing the signal"
        case .exclusiveModeUnavailable:
            "Exclusive audio mode is not available"
        case .bluetoothCompression:
            "Bluetooth audio compression is active"
        case .airplayCompression:
            "AirPlay compression is active"
        case .dspProcessingEnabled:
            "Digital signal processing is enabled"
        case .equalizerActive:
            "Audio equalizer is active"
        case .spatialAudioActive:
            "Spatial audio processing is active"
        case .unknownIssue:
            "Unknown issue preventing bit-perfect playback"
        }
    }
    ...
    public var technicalDescription: String {
        switch self {
        case .sampleRateMismatch:
            "Source and output sample rates differ, requiring resampling"
        case .bitDepthMismatch:
            "Source and output bit depths differ, requiring bit depth conversion"
        case .channelCountMismatch:
            "Source and output channel counts differ, requiring channel matrix processing"
        case .deviceNotCapable:
            "The audio output device hardware does not support the source format natively"
        case .systemVolumeNotUnity:
            "System volume level is not 100%, causing digital volume scaling"
        case .applicationVolumeNotUnity:
            "Application volume level is not 100%, causing digital volume scaling"
        case .audioProcessingActive:
            "One or more audio processing stages are active in the signal path"
        case .formatConversionRequired:
            "The audio format requires conversion to match output capabilities"
        case .systemMixerActive:
            "The system audio mixer is processing multiple audio streams"
        case .exclusiveModeUnavailable:
            "Exclusive mode access to the audio device is not available"
        case .bluetoothCompression:
            "Bluetooth audio codecs are compressing the audio signal"
        case .airplayCompression:
            "AirPlay protocol is compressing the audio signal"
        case .dspProcessingEnabled:
            "Hardware or software DSP processing is enabled"
        case .equalizerActive:
            "Audio equalizer is applying frequency response modifications"
        case .spatialAudioActive:
            "Spatial audio processing is virtualizing the soundstage"
        case .unknownIssue:
            "An unidentified issue is preventing bit-perfect audio playback"
        }
    }
