...
struct PlaylistDetailView: View {
    ...
    private func formattedDuration(for tracks: [Track]) -> String? {
        ...
        formatter.allowedUnits = totalDuration >= 3600 ? [.hour, .minute] : [.minute, .second]
    ...
    private func sortTracks(_ tracks: [Track], preserveManualOrder: Bool) -> [Track] {
        ...
        case .manual:
            preserveManualOrder ? tracks : tracks
        case .dateAdded:
            tracks.sorted { $0.dateAdded > $1.dateAdded }
        case .dateModified:
            tracks.sorted { $0.dateModified > $1.dateModified }
        case .title:
            tracks.sorted { $0.title.localizedCaseInsensitiveCompare($1.title) == .orderedAscending }
        case .artist:
            tracks.sorted { $0.artist.localizedCaseInsensitiveCompare($1.artist) == .orderedAscending }
        case .album:
            tracks.sorted { $0.album.localizedCaseInsensitiveCompare($1.album) == .orderedAscending }
        case .duration:
            tracks.sorted { $0.duration > $1.duration }
        case .playCount:
            tracks.sorted { $0.playCount > $1.playCount }
        case .rating:
            tracks.sorted { ($0.rating ?? 0) > ($1.rating ?? 0) }
        case .sampleRate:
            tracks.sorted { $0.sampleRate > $1.sampleRate }
        case .random:
            tracks.shuffled()
        }
    }
    ...
    private func evaluate(rule: SmartPlaylistRule, for track: Track) -> Bool {
        switch rule.field {
        case .title:
            evaluateString(track.title, rule: rule)
        case .artist:
            evaluateString(track.artist, rule: rule)
        case .album:
            evaluateString(track.album, rule: rule)
        case .genre:
            evaluateString(track.genre, rule: rule)
        case .year:
            evaluateNumeric(track.year.map(Double.init), rule: rule)
        case .duration:
            evaluateNumeric(track.duration, rule: rule)
        case .playCount:
            evaluateNumeric(Double(track.playCount), rule: rule)
        case .rating:
            evaluateNumeric(track.rating.map(Double.init), rule: rule)
        case .dateAdded:
            evaluateDate(track.dateAdded, rule: rule)
        case .lastPlayed:
            evaluateOptionalDate(track.lastPlayed, rule: rule)
        case .audioFormat:
            evaluateString(track.audioFormat, rule: rule)
        case .sampleRate:
            evaluateNumeric(track.sampleRate, rule: rule)
        case .bitDepth:
            evaluateNumeric(Double(track.bitDepth), rule: rule)
        case .isLossless:
            evaluateBool(track.isLossless, rule: rule)
        case .isFavorite:
            evaluateBool(track.isFavorite, rule: rule)
        case .fileSize:
            evaluateNumeric(Double(track.fileSize), rule: rule)
        }
    }
    ...
    private func evaluateBool(_ actual: Bool, rule: SmartPlaylistRule) -> Bool {
        ...
        case .isFalse:
            !actual
        case .equals:
            actual == (rule.value.lowercased() == "true")
        case .notEquals:
            actual != (rule.value.lowercased() == "true")
        default:
            false
    ...
    private func evaluateDate(_ actual: Date, rule: SmartPlaylistRule) -> Bool {
        ...
        case .inTheLast, .notInTheLast:
            ...
            let threshold = Date().addingTimeInterval(-days * 86400)
            let comparison = actual >= threshold
            return rule.operator == .inTheLast ? comparison : !comparison
        case .greaterThan:
            guard let days = Double(rule.value) else { return false }
            let comparisonDate = Date().addingTimeInterval(-days * 86400)
